...
class LangGraphAgent:
    ...
    def __init__(self):
        ...
        # Initialize LLM without tools first
        self.llm = ChatOpenAI(
        ...
        # Only bind tools for models that support them
        # Some models on OpenRouter don't support tools
        self.supports_tools = not any(x in settings.LLM_MODEL.lower() for x in ['qwen', 'llama', 'mistral', 'gemma'])
        
        if self.supports_tools:
            self.llm = self.llm.bind_tools(tools)
            self.tools_by_name = {tool.name: tool for tool in tools}
            logger.info("llm_tools_enabled", model=settings.LLM_MODEL, tools_count=len(tools))
        else:
            self.tools_by_name = {}
            logger.info("llm_tools_disabled", model=settings.LLM_MODEL, reason="Model doesn't support tools")
    ...
    def _should_continue(self, state: GraphState) -> Literal["end", "continue"]:
        ...
        """
        # If model doesn't support tools, always end
    ...
    async def get_response(
        ...
        config = {
            ...
            "callbacks": [CallbackHandler()] if LANGFUSE_AVAILABLE else [],
            "metadata": {
                "user_id": user_id,
