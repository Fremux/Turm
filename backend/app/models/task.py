...
class TaskItem(Base):
    ...
    category_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("task_categories.id", ondelete="CASCADE"), nullable=False
    )
    name: Mapped[str] = mapped_column(Text, nullable=False)
...
class TaskRouting(Base):
    ...
    task_item_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("task_items.id", ondelete="CASCADE"), primary_key=True
    )
    handled_by_id: Mapped[str] = mapped_column(
        String, ForeignKey("org_nodes.id", ondelete="RESTRICT"), nullable=False
    )
    escalate_to_id: Mapped[Optional[str]] = mapped_column(
        String, ForeignKey("org_nodes.id", ondelete="SET NULL")
    )
